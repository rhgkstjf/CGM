...
import pandas as pd
import math
import numpy as np     # pandas의 pd.Serise()함수를 사용할때 numpy의 array가 인자로 입력되므로 임포트한다.
...
def readData(filePath) :
    ...
    while True:
        ...
        newPointx.append(float(tmp_glucose))
...
filename = os.listdir("sch")  # 해당 디렉토리("sch")내부의 모든 파일 리스트들을 불러와 저장한다.

train_data_name = []
test_data_name = []

# for문으로 text_data와 test_data를 구분한다.
for fn in filename:
    if fn.find("test") != -1:
...
# for문으로 train_x_data와 train_y_data를 구분한다.
...
# for문으로 text_x_data와 test_y_data를 구분한다.
test_x_data = []
test_y_data = []
...
w1 = tf.Variable(tf.random_uniform([9,15],minval = -1 , maxval = 1,dtype=tf.float32))
